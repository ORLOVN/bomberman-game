...
const storeStuffing = async (req: Request) => {
  ...
  if (state.auth.role === Roles.user) {
    await store.dispatch(
      themeApiService.endpoints.getUserTheme.initiate(state.auth.user.id)
    );
    ...
    if (forum === "forum" && topic === "topic") {
      await store.dispatch(
        forumApiService.endpoints.getTopic.initiate(topicId)
      );
    }
    await store.dispatch(
      leaderBoardApiService.endpoints.getScoreEntries.initiate({
