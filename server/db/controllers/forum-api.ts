...
export class ForumAPI {
  public static findTopic = async (
    ...
    return forumTopicService
      ...
      .then(async (topic) => {
        ...
        const yaUser = await YaUserInfoAPI.getInfo(topic.yaId, request.cookies);

        const comments = (
          await forumCommentService.findByTopicId(request.params.topicId)
        ).map((t) => t.dataValues);
        ...
        const yaUsers = await Promise.all(
          yaIds.map((authorYaId) =>
            YaUserInfoAPI.getInfo(authorYaId, request.cookies)
          )
        );
        ...
        const formatComment = (
          comment: IComment & { createdAt: string; updatedAt: string }
        ): ITransformedComment => ({
          id: comment.id,
        ...
        const rootComments: (ReturnType<typeof formatComment> & {
        ...
        comments.forEach((c) => {
          (c.parentCommentId === null ? rootComments : childComments).push(
            formatComment(c)
          );
        });
  ...
  public static findAllTopics = async (
    ...
    return forumTopicService
      ...
      .then(async (res) => {
        ...
        const yaUsers = await Promise.all(
          yaIds.map((yaId) => YaUserInfoAPI.getInfo(yaId, request.cookies))
        );
  ...
  public static createTopic = async (request: Request, response: Response) => {
    const { yaId, title, body } = request.body;
  ...
  public static createComment = async (
    request: Request<ICreateCommentRequest>,
    response: Response
  ) => {
